...
\subsubsection{32-bit stack elements}\label{sec:winternitz-31-bit-integers}

Instead of bitwise operations, script can use arithmetic ones, but the
Bitcoin Script supports opcodes such as \texttt{OP\_ADD}, only for
32-bit signed integers, and as the last one is reserved for a sign,
only 31 bits can be used to store the state. This could be considered
as limitation, but as most of the complex arithmetic operations in
BitVM are done using only 32-bit integers, we can easily fix $\ell$ as
$\ell = 32$ for all intermidiate states.

By defining the $\mathsf{Enc}(m)$ as a domination free function
$P(m)$, like described in~\cite{applied-crypto}, $d+1$ becomes the
power of two which splits $m$ by some number of equal chunks of
$\log_2{(d+1)}$ bits, and $k$ becomes the sum of $n_0$ --- the number of
...
bits = lambda d: math.log(d+1, 2)
bits = lambda d: math.ceil(math.log(d+1, 2))
n0 = lambda bits: math.ceil(l / bits)
n1 = lambda bits,n0: math.ceil((2**(bits) * n0).bit_length() / bits)
k = lambda n0, n1: n0 + n1
...
If chunks are equal lengthes, the recovery from $n_0$ digits is
simply as:

\begin{equation}
  m = \sum_{i = 0}^{n_0} e_i \cdot 2^{i \log_2{(d+1)}}
...
\begin{empheqboxed}
  \begin{align*}
    \elem{e_j} \underbrace{\opcode{OP\_DUP} \opcode{OP\_ADD}}_{n \; times}
  \end{align*}
\end{empheqboxed}
...
size. Additionaly, for calculating the lookup table for signature
verification, $2d \cdot k$ opcodes are used. Further more, for message
recovery $2 \sum_{i = 0}^{n_0} i \log_2{(d+1)}$ number of opcodes are
added too. So the total script size, excluding utility opcodes, will
be at least $40 * k + 2 d k + 2 \sum_{i = 0}^{n_0} i \log_2{(d+1)}$ (the
  sizes for different $d$ can be seen in
table~\ref{tab:winternitz-script-size}).
...
for d in ds:
    ...
    ver_size = 2 * d * k_
    rec_size = 0
    for i in range(0, n0_):
        rec_size += int(i * math.log(d+1, 2))
