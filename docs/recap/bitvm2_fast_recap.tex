...
\author{Kyrylo Baibula}
\date{\today}
\title{BitVM fast recap}
\hypersetup{
  pdfauthor={Kyrylo Baibula},
...
\textbf{give others the opportunity} to \textbf{disprove this
fact}. In our case, this means giving challengers the ability to
prove that at least one of the sub-programs \(f_i(z_{i-1}) \neq z_i\).
...
space \(\mathcal{M} = {\{0, 1\}}^{l}\) of \(l\) -bit messages.
