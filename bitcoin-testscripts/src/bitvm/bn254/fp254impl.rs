use crate::bitvm::bigint::add::limb_add_carry;
use crate::bitvm::bigint::bits::limb_to_be_bits;
use crate::bitvm::bigint::sub::limb_sub_borrow;
use crate::bitvm::bigint::u29x9::{
    u29x9_mul_karazuba, u29x9_mul_karazuba_imm, u29x9_mulhi_karazuba_imm, u29x9_mullo_karazuba_imm,
    u29x9_square,
};
use crate::bitvm::bigint::U254;
use crate::bitvm::bn254::fq::Fq;
use ark_ff::{BigInteger, PrimeField};
use bitcoin_script::script;
use bitcoin_utils::pseudo::OP_256MUL;
...
pub trait Fp254Impl {
    ...
    fn is_one(a: u32) -> Script {
        ...
        for (i, limb) in u29x9_one.iter_mut().enumerate() {
            *limb = *montgomery_one
                .clone()
    ...
    fn mul_by_constant(constant: &Self::ConstantType) -> Script {
        ...
        let mut u29x9_montgomery = [0u32; 9];
        {
            ...
            for (i, limb) in u29x9_montgomery.iter_mut().enumerate() {
                *limb = *constant
                    .clone()
                    .div(BigUint::one().shl(29 * i) as BigUint)
                    .rem(BigUint::one().shl(29) as BigUint)
