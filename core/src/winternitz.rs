...
pub fn public_key_for_digit(mut secret_key: Vec<u8>, digit_index: u32) -> [u8; 20] {
    secret_key.push(digit_index as u8);

    let mut hash = hash160::Hash::hash(&secret_key);
...
pub fn generate_public_key(secret_key: Vec<u8>) -> PublicKey {
    let mut public_key_array = [[0u8; 20]; N as usize];
    for i in 0..N {
        public_key_array[i as usize] = public_key_for_digit(secret_key.clone(), i);
    }
    public_key_array
}

/// Compute the signature for the i-th digit of the message
pub fn digit_signature(mut secret_key: SecretKey, digit_index: u32, message_digit: u8) -> Script {
    secret_key.push(digit_index as u8);
...
pub fn sign_digits(secret_key: SecretKey, message_digits: [u8; N0 as usize]) -> Script {
    ...
    script! {
        for i in 0..N {
            { digit_signature(secret_key.clone(), i, checksum_digits[ (N-1-i) as usize]) }
        }
    }
}
...
mod test {
    ...
    fn test_winternitz() {
        ...
        let secret_key = Vec::<u8>::from_hex(MY_SECKEY).unwrap();
        let public_key = generate_public_key(secret_key.clone());

        let script = script! {
            { sign_digits(secret_key.clone(), MESSAGE) }
        ...
        run(script! {
            { sign_digits(secret_key, MESSAGE) }
            { checksig_verify(&public_key) }
